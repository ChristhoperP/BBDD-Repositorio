...
for ($i = 0; $i < sqlsrv_num_rows($tablas); $i++) {
    ...
    if (isset($_SESSION['consulta-' . $tabla_actual["name"]])) {
        ...
        if(!$conexionOLTP->ejecutarConsulta($vista)){
            $reporte_final[$i]="Error en la consulta de la tabla: ".$tabla_actual["name"];
        ...
        if(!$conexionOLAP->ejecutarConsulta($cursor)){
            $reporte_final[$i]="Error al crear el cursor de la tabla: ".$tabla_actual["name"];
        ...
        if(!$resultado_cursor=$conexionOLAP->ejecutarConsulta($consulta_ejecutar)){
            $reporte_final[$i]="Error al ejecutar el cursor de la tabla: ".$tabla_actual["name"];
            //echo '{"mensaje":"Error al ejecutar el cursor."}';
            //return;
        }else{
            //echo '{"mensaje":"'.sqlsrv_fetch_array($resultado_cursor)[0].'"}';
            $reporte_final[$i]="SE INSERTARON LOS REGISTROS EXITOSAMENTE EN LA TABLA: ".strtoupper($tabla_actual["name"]);
